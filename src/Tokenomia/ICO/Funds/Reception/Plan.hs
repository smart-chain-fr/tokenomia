...
transition
    ...
    where
        ...
        accumulatedFundsToSent = sumAdaFunds (receivedFunds <$> toAscList commands)
...
sumAdaFunds :: [ReceivedFunds] -> Ada
sumAdaFunds [] = Ada.lovelaceOf 0
sumAdaFunds (ReceivedFunds {funds = Left _} : xs)  = sumAdaFunds xs
sumAdaFunds (ReceivedFunds {funds = Right x} : xs) = x + sumAdaFunds xs
