...
data RawReceivedFundsByTx = RawReceivedFundsByTx
  { txId :: TxId
  , inputAdresses :: NonEmpty Address
  , sources :: NonEmpty WalletUTxO
  }
  deriving stock (Show)

data RejectedTx = TxMadeFromOutsideTheICORoundWallet
  { tx :: RawReceivedFundsByTx
  , externalAddresses :: NonEmpty Address
  }
  deriving stock (Show)

data AuthentifiedFunds = AuthentifiedFunds
  { source :: WalletUTxO
  , adas :: Ada
  , receivedAt :: Slot
...
instance Show AuthentifiedFunds where
  show AuthentifiedFunds {..} =
    "\n" <> (showTxOutRef . txOutRef . utxo) source
      <> "\n   | index : "
      <> (show @Integer . fromIntegral) index
      <> "\n   | received at : "
      <> show (getSlot receivedAt)
      <> "\n   | amount  : "
      <> (show . getLovelace) adas
      <> " Lovelaces"
      <> "\n   | payback Address  : "
      <> show paybackAddress
...
authentifyTx
  round@RoundSettings
    { investorsWallet = Wallet {name = currentRoundWallet}
    , previousRoundMaybe =
...
fetchRawReceivedFundsByTx RoundSettings {addresses = RoundAddresses {exchange = IndexedAddress {childAddressRef = exchangeAddress}}} =
  do
    ...
    let getTxOutRefId = txOutRefId . txOutRef . utxo
        walletUtxosGroupedByTx =
          (\(txs :: NonEmpty WalletUTxO) -> ((getTxOutRefId . NEL.head) txs, txs))
            <$> NEL.groupWith1 getTxOutRefId walletUtxos

    prj <- B.projectFromEnv''
    utxosAdresses <-
      liftIO (B.runBlockfrost prj $ fetchUTxOsAddresses' $ fst <$> walletUtxosGroupedByTx)
        >>= ( \case
                Left e -> throwError $ BlockFrostError e
                Right res -> return res
            )
...
fetchUTxOAddresses' txId = do
  addresses <-
    (\(x :: B.TransactionUtxos) -> Address . T.unpack . coerce . B._utxoInputAddress <$> B._transactionUtxosInputs x)
