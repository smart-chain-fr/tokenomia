...
data InvestorPlan = InvestorPlan {investorRef :: WhiteListedInvestorRef, commands :: OSet Command} deriving stock (Show, Eq)
...
transition
  ...
  State {settings = settings@Settings {..}, volumes = volumes@AddressVolumes {..}, ..}
    ...
    | adressSaturatedWithIncomingFund =
      appendCommand $
