...
transfer' walletName receiverAddr utxoWithToken amount labelMaybe = do
    ...
    ChildAddress {address = senderWalletChildAddress} <- fetchById (ChildAddressRef walletName 0)
    ...
    submit
      TxBuild
        ...
        , outputs = ToWallet receiverAddr valueToTransfer 
                :| [ToWallet senderWalletChildAddress change]
