...
singleTransfer ::
  ...
  Fees ->
  Parameters ->
  (ChildAddressIndex, Distribution) ->
  m ()
singleTransfer fees parameters (index, distribution) =
  do
...
singleTransferInputs ::
  ...
  Parameters ->
  ChildAddressIndex ->
  m (Maybe (NonEmpty TxInFromWallet))
