...
fetchDerivedChildAddressIndexes name =
  do
    ...
    liftIO $
      (fmap.fmap.fmap)
        (ChildAddressIndex . read @Integer . last . splitOn "/" . C.unpack)
        NEL.nonEmpty
        . tail
        <$> (find childAddressesPath "-type" "d" |> captureWords) -- remove ./
        -- return ./ ./0 ./1
    >>= \case
...
generateChildAddressFile childAddressRef@ChildAddressRef {name, index = (ChildAddressIndex indexInteger)} fileType = do
  ...
  case fileType of
    AddressTxt ->
      getFilePath AddressTxt >>= \path -> do
    ...
    ExtendedPublicKeyJSON ->
      getFilePath ExtendedPublicKeyJSON >>= \path -> do
        ...
        liftIO $
          cardano_cli
            ...
            "verification-key"
            "--signing-key-file"
            extendedPrivateKeyJSONPath
            "--verification-key-file"
            path
    ExtendedPublicKeyTxt ->
      getFilePath ExtendedPublicKeyTxt >>= \path -> do
        extendedPrivateKeyTxtPath <- getFilePath ExtendedPrivateKeyTxt
        liftIO $
          (cat extendedPrivateKeyTxtPath |> cardano_address "key" "public" "--with-chain-code")
            &> (Truncate . fromString) path
    ExtendedPrivateKeyJSON ->
      getFilePath ExtendedPrivateKeyJSON >>= \path -> do
        extendedPrivateKeyTxtPath <- getFilePath ExtendedPrivateKeyTxt
        liftIO $
          cardano_cli
            "key"
